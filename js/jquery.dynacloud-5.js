...
// Adapted from <http://www.perseus.tufts.edu/Texts/engstop.html>
 ...
 stopwords: [ "a", "about", "above", "accordingly", "after",
  ...
  "yes", "your", "yours", "yourself", "yourselves", "this" ]
...
jQuery.fn.dynaCloud = function(outElement) {
 ...
 return this.each(function() {
  ...
  out.empty();
  //alert("cl.length:" + cl.length);
  //temp = "";
  var l = jQuery.dynaCloud.max == -1 ? cl.length : Math.min(jQuery.dynaCloud.max, cl.length);

  for (var i = 0; i < l; ++i) {
   //temp += '<a href="#' + cl[i].el + '" style="font-size: ' + Math.ceil((cl[i].count / max) * jQuery.dynaCloud.scale) + 'em"><span>' + cl[i].el + '</span></a> &nbsp; ';
   out.append('<a href="#' + cl[i].el + '" title="' + cl[i].count + '" style="font-size: ' + Math.ceil((cl[i].count / max) * jQuery.dynaCloud.scale) + 'em"><span>' + cl[i].el + '</span></a> &nbsp; ');
  }
  //alert(temp);
  var target = this;
  ...
  jQuery('a', out).each(function() {
   jQuery(this).click(function() { 
    ...
    //jQuery(target).each(function() {
     jQuery("body").highlight(text); //this
    //});
    return false;
   });
  });
