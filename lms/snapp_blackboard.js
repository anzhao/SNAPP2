function PerformSocialAnalysisBlackboardV10()
{
...
function PerformSocialAnalysisBlackboardV9()
{
		 ...
		 var tmp;
		 var reply_to;
		 var scu = jQuery("span[editedTimeInBBFormat]").each(function(){
         tmp = jQuery(this).attr("editedTimeInBBFormat");
});	
		 
		  //for(var k=0; k<scu.length; k++)
	    //{
			
			//start to process line by line
			var divEls;
			var layer;
		    var cbr = jQuery(".msg-nexus");
			var adl = jQuery(".profileCardAvatarThumb");
			for (var i=0; i<cbr.length; i++)
			{
				 divEls = cbr[i].getElementsByTagName("div");
			     layer = divEls[0].id;
				 threaddepth = getTextAfterChar(layer,'_').substring(1);
				 //alert("THE VALUE VALUE VALUE OF THREADDEPTH ......" + threaddepth);
				 posted_by = adl[i].innerHTML;
				 posted_by = posted_by.substring(posted_by.lastIndexOf('>')+1,posted_by.length);
				 posted_by = stripSpaces(posted_by);
				 //alert("POSTED_BY IS: " + posted_by);
				  if (forumusers[posted_by])
				{
					forumusers[posted_by] = forumusers[posted_by] + 1;
				}
				else
				{
				 ...
				 threadowners[threaddepth] = posted_by;
				 posted_on = "9/12/07 11:11 AM";
				 if (threaddepth.length == 1 || threaddepth.length == 2)
                 {
                   reply_to = "-";
                 }
                 else 
                 {
	               //alert("THE LENGTH OF THREADOWNERS: "+ threadowners.length);
	               threadReplyDepth = threaddepth.substring(0, threaddepth.lastIndexOf('_'));
	  
	               //for (var m=0; m < threadowners.length; m++) {
                   //alert("The value we are investigating: " + threadowners[m]);
                   //}
	               ...
	               {
		              alert("STRANGE, reply_to VALUE HAS NOT been passed by threadsowners!!!");
	...
	//alert("The number of the total posts is: " + totalposts);
}
