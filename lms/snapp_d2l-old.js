...
function retrieveD2LForumThread() {
  ...
  var html = jQuery.ajax({
    url: threadURL, success: function (html) {
      //alert("AJAX SUCCESS!");
...
function retrieveD2LForumPageThread() {
  //alert(globalD2LSubPageList.length);
  if (globalD2LSubPageListIndex == (globalD2LSubPageList.length - 1)) {
    D2LMultiForumMinerCompleted();
  }
  else {
    ...
    var html = jQuery.ajax({
      url: threadURL, async: false, success: function (html) {
