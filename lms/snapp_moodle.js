...
function PerformSocialAnalysisMoodle2() {
  ...
  for (i = 0; i < allForumPostsTables.length; i++) {
    ...
    posted_on = posted_on.substring(posted_on.lastIndexOf('-') + 2, posted_on.length);
    posted_on = posted_on.substring(posted_on.indexOf(',') + 2, posted_on.length);
    posted_on = posted_on.substring(0, posted_on.lastIndexOf(','));

    if (document.documentElement.lang == "fr") {
      posted_on = ConvertMonth(posted_on);
    }
    //alert(posted_on);
    //var posted_onAsDateObj = Date.parse(posted_on);
    //need to get date in format like  Monday, 3 January 2011


    //posted_on = posted_onAsDateObj.getDate() + " " + posted_onAsDateObj.getMonthName() + " " + posted_onAsDateObj.getFullYear();
    //posted_on += ", 01:55 AM";
    ...
    post_idLinkObj = jQuery('a[id*="p"]', jQuery(currentPost).parent());
    reply_idLinkObj = jQuery('a[href*="discuss.php?d="]', commandsObj[0]);

    post_id = 0;
    reply_id = 0;

    post_id = post_idLinkObj[0].id;
    post_id = post_id.substring(post_id.indexOf('p') + 1, post_id.length);
    ...
    //alert("posted_on: " + posted_on + " posted_by:" + posted_by + " postid:" + post_id + " replyid:" + reply_id);
...
function ConvertMonth(frdates) {
  ...
  frdates = frdates.replace("f�vrier", "February");
  ...
  frdates = frdates.replace("ao�t", "August");
  ...
  frdates = frdates.replace("d�cembre", "December");
